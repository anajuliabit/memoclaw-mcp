...
const TOOLS = [
  ...
  },
  {
    name: 'memoclaw_list',
    description:
      'List memories chronologically (newest first). Use this for:\n' +
  ...
  },
  {
    ...
    description:
      'Permanently delete a single memory by its ID. This cannot be undone. ' +
  ...
  },
  {
    ...
    description:
      'Delete multiple memories at once by their IDs. This cannot be undone. ' +
  ...
  },
  {
    ...
    description:
      'Update an existing memory by its ID. Only the provided fields are changed — omitted fields stay the same. ' +
  ...
  },
  {
    ...
    description:
      'Bulk-ingest a conversation or raw text. The server extracts facts, deduplicates against existing memories, ' +
  ...
  },
  {
    ...
    description:
      'Merge similar/duplicate memories by clustering. Reduces memory clutter and combines redundant information. ' +
  ...
  },
  {
    ...
    description:
      'Get proactive memory suggestions: stale memories that may need refreshing, fresh/hot memories trending up, ' +
  ...
  },
  {
    ...
    description:
      'Create a directed relationship between two memories. Relations form a knowledge graph ' +
  ...
  },
  {
    ...
    description:
      'List all relationships for a specific memory (both incoming and outgoing). ' +
  ...
  },
  {
    ...
    description:
      'Run multiple semantic searches in a single API call. More efficient than calling memoclaw_recall ' +
  ...
  },
  {
    ...
    description:
      'Import memories from a JSON array. This is the inverse of memoclaw_export — ' +
  ...
  },
  {
    name: 'memoclaw_graph',
    description:
      'Traverse the memory graph starting from a specific memory and find related memories through relation chains. ' +
  ...
  try {
    switch (name) {
      ...
      case 'memoclaw_batch_recall': {
        const { queries, namespace, tags, memory_type } = args as any;
      ...
      case 'memoclaw_import': {
        const { memories, namespace, skip_duplicates } = args as any;
        if (!memories || !Array.isArray(memories) || memories.length === 0) {
          throw new Error('memories is required and must be a non-empty array');
        }
      ...
      case 'memoclaw_graph': {
        const { memory_id, relation_types, direction, depth, limit } = args as any;
        if (!memory_id) {
        ...
        async function getRelations(memId: string): Promise<any[]> {
          try {
            ...
            if (relation_types && Array.isArray(relation_types) && relation_types.length > 0) {
              rels = rels.filter((r: any) => relation_types.includes(r.relation_type));
            }
            
            // Filter by direction
            if (direction === 'outgoing') {
              rels = rels.filter((r: any) => r.source_id === memId);
            } else if (direction === 'incoming') {
              rels = rels.filter((r: any) => r.target_id === memId);
            }
        ...
        const relatedMemories: { memory: any, depth: number, relation: any }[] = [];
        const queue: { id: string, depth: number }[] = [{ id: memory_id, depth: 0 }];
        ...
        while (queue.length > 0 && relatedMemories.length < maxResults) {
          ...
          for (const rel of relations) {
            ...
            if (direction === 'outgoing' || !direction) {
              if (rel.source_id === current.id) targetId = rel.target_id;
            }
            if (direction === 'incoming' || !direction) {
              if (rel.target_id === current.id) targetId = rel.source_id;
            }
