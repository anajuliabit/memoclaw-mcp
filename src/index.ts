    ...
    switch (name) {
      ...
      case 'memoclaw_graph': {
        ...
        if (relation_types && Array.isArray(relation_types) && relation_types.length > 0) {
          relations = relations.filter((r: any) => relation_types.includes(r.relation_type));
        }
        
        // Filter by direction
        if (direction === 'outgoing') {
          relations = relations.filter((r: any) => r.source_id === memory_id);
        } else if (direction === 'incoming') {
          relations = relations.filter((r: any) => r.target_id === memory_id);
        }
        ...
        const targetIds = direction === 'incoming' 
          ? relations.map((r: any) => r.source_id)
          : relations.map((r: any) => r.target_id);
        ...
        for (const id of targetIds) {
          if (!visited.has(id)) {
            ...
            if (relatedMemories.length >= (limit || 20)) break;
          }
        }
        ...
        const formatted = relatedMemories.map((m: any) => formatMemory(m)).join('\n\n');
        return { 
          content: [{ 
            type: 'text', 
