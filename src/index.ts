...
function validateContentLength(content: string, label = 'content'): void {
...
const TOOLS = [
  ...
  },
  {
    name: 'memoclaw_pin',
    description:
      '📌 Pin a memory to prevent it from decaying over time. ' +
  ...
  },
  {
    name: 'memoclaw_unpin',
    description:
      '📌 Unpin a memory, re-enabling normal decay behavior. ' +
  ...
  },
  {
    ...
    description:
      'Update multiple memories in a single call. Each update specifies a memory ID and the fields to change. ' +
  ...
  },
  {
    ...
    description:
      '⭐ Get your most important memories — high importance, frequently accessed, or pinned. ' +
  ...
  },
  {
    name: 'memoclaw_stats',
    description:
      '📊 Get memory usage statistics for your wallet. Returns aggregate stats including total memories, ' +
  ...
  try {
    switch (name) {
      ...
      case 'memoclaw_core_memories': {
        const { limit, namespace, agent_id } = args as any;
        const params = new URLSearchParams();
        if (limit !== undefined) params.set('limit', String(limit));
        if (namespace) params.set('namespace', namespace);
        if (agent_id) params.set('agent_id', agent_id);
